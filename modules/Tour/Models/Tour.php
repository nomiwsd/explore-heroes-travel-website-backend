...
class Tour extends Bookable
{
    ...
    public function getNumberServiceInLocation($location)
    {
        $number = 0;
        if (!empty($location)) {
            $number = \DB::table('bravo_tour_locations')
                ->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_tour_locations.location_id', '=', 'bravo_locations.id')
                        ->where('bravo_locations._lft', '>=', $location->_lft)
                        ->where('bravo_locations._rgt', '<=', $location->_rgt);
                })
                ->join('bravo_tours', 'bravo_tours.id', '=', 'bravo_tour_locations.tour_id')
                ->where('bravo_tours.status', 'publish')
                ->distinct('bravo_tours.id')
                ->count('bravo_tours.id');
...
//            })->where("bravo_tours.status", "publish")->with(['translation'])->count("bravo_tours.id");
    ...
    public function search($request)
    {
        ...
        if (!empty($location_id = $request['location_id'] ?? "" )) {
            ...
            if (!empty($location)) {
                $model_Tour->whereHas('locations',function($q) use($location){
