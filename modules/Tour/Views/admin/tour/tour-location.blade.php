        ...
        <div class="row">
            <div class="col-md-9">
                @if(is_default_lang())
                    ...
                    {{--            </div>--}}
                    @php
                        $selectedLocationIds = old('location_ids', $row->exists ? $row->locations->pluck('location_id')->toArray() : []);
                    @endphp
                    <div class="form-group">
                        <label>{{ __('Locations') }}</label>
                        <select name="location_ids[]" multiple="multiple" class="form-control multiple-select " style="width:100%">
                                <?php

                                $traverse = function ($locations, $prefix = '') use (&$traverse, $row,$selectedLocationIds) {
                                    foreach ($locations as $location) {
                                        $selected = in_array($location->id, $selectedLocationIds) ? 'selected' : '';
                                        printf("<option value='%s' %s>%s</option>", $location->id, $selected, $prefix . ' ' . $location->name);
                                        $traverse($location->children, $prefix . '-');
                                    }
                                };
            ...
            <div class="col-md-3">
                <div class="form-group">
                    {!! \Modules\Media\Helpers\FileHelper::fieldUpload('map_image_id',$row->map_image_id) !!}
                </div>
            </div>
        </div>
        ...
        @if(is_default_lang())
            <div class="form-group">
                ...
                <div class="control-map-group">
                    ...
                    <input type="text" placeholder="{{__("Search by name...")}}" class="bravo_searchbox form-control"
...
{{--    <div class="panel-title"><strong>{{__("Tour Locations")}}</strong></div>--}}
{{--    <div class="panel-body">--}}
{{--        @include('Tour::admin/tour/locations')--}}
{{--    </div>--}}
{{--</div>--}}
