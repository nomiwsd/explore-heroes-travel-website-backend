...
class TourController extends AdminController
{
    ...
    {
        $this->setActiveMenu('/admin/module/tour/tour');
    ...
    public function index(Request $request)
    {
        ...
        if ($request->wantsJson() || $request->expectsJson()) {
            return response()->json($query->with(['author', 'category_tour'])->paginate(20));
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($request->wantsJson() || $request->expectsJson()) {
            return response()->json([
                'data' => $row->load(['author', 'category_tour', 'locations', 'tour_term']),
...
//                unset($locations['__number__']);
//                foreach ($locations as $location) {
//                    $new_location = new TourLocation();
//                    $new_location->tour_id = $row->id;
//                    $new_location->location_id = $location['location_id'];
//                    $new_location->image_id = $location['image_id'];
//                    $new_location->save();
//                }
