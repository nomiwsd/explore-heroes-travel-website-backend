...
class EnquiryController extends AdminController
{
    ...
    public function tailorMadeTourIndex(Request $request)
    {
        $this->checkPermission('enquiry_view');
        $query = TailorMadeTour::query();
        if (!empty($request->s)) {
            $query->where('email', 'LIKE', '%' . $request->s . '%');
            $query->orderBy('email', 'asc');
            $title_page = __('Search results: ":s"', ["s" => $request->s]);
        ...
        $data = [
            ...
            'breadcrumbs' => [
                [
                    ...
                    'url'  => route('report.admin.enquiry.tailorMadeTourIndex')
                ],
                [
                    'name'  => __('All'),
                    'class' => 'active'
            ...
            'enquiry_update'        => $this->hasPermission('enquiry_update'),
            'enquiry_manage_others' => $this->hasPermission('enquiry_manage_others'),
            'page_title'=> $title_page ?? __("Tailor Made Tour Management")
        ];

        return view('Report::admin.enquiry.tailor-made-tour', $data);
    }
